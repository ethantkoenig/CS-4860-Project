...
COQLIBS?= -R cpdt/src Cpdt\
  -R arrays Arrays\
  -R naturals Naturals
COQDOCLIBS?=-R cpdt/src Cpdt\
  -R arrays Arrays\
  -R naturals Naturals
...
VOFILES:=$(VFILES:.v=.vo)
VOFILES2=$(patsubst arrays/%,%,$(filter arrays/%,$(VOFILES)))
VOFILES3=$(patsubst naturals/%,%,$(filter naturals/%,$(VOFILES)))
...
install:
	cd arrays; for i in $(VOFILES2); do \
	 install -d `dirname $(DSTROOT)$(COQLIBINSTALL)/Arrays/$$i`; \
	 install -m 0644 $$i $(DSTROOT)$(COQLIBINSTALL)/Arrays/$$i; \
	done
	cd naturals; for i in $(VOFILES3); do \
	 install -d `dirname $(DSTROOT)$(COQLIBINSTALL)/Naturals/$$i`; \
	 install -m 0644 $$i $(DSTROOT)$(COQLIBINSTALL)/Naturals/$$i; \
	done
